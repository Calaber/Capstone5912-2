...
public class AVProMovieCaptureEditorWindow : EditorWindow 
{
	...
	private void Configure(AVProMovieCaptureBase capture)
	{
		...
		capture._useMediaFoundationH264 = _useMediaFoundationH264;
	...
	private void CreateComponents()
	{
		...
		_audioCapture = null;
		if (_captureModeIndex == 0 && _captureAudio && _audioDeviceIndex == 0)
		{
	...
	void OnEnable()
	{
		...
		// Check that the plugin version number is not too old
		{
			...
			if (pluginVersion < AVProMovieCaptureBase.ScriptVersion)
			{
				_pluginVersionWarningText = "Warning: Plugin version number " + pluginVersion.ToString("F2") + " is older than script version number " + AVProMovieCaptureBase.ScriptVersion.ToString("F2") + ".  It looks like the plugin didn't upgrade correctly.  To resolve this please restart Unity and try to upgrade the package again.";
			}
		}
	}
	...
	private void DrawCapturingGUI()
	{
		...
		if (!_capture._useMediaFoundationH264)
		{
	...
	private void DrawConfigGUI_Visual()
	{
		...
		GUILayout.Space(8f);
		if (ShowConfigList("Codec", _videoCodecNames, _videoCodecConfigurable, ref _videoCodecIndex, true, !_useMediaFoundationH264))
		{
		...
		GUILayout.BeginHorizontal();
		_useMediaFoundationH264 = EditorGUILayout.Toggle("Use MF MP4 encoder", _useMediaFoundationH264);
		GUILayout.Label("(experimental)");
		GUILayout.EndHorizontal();
		if (_useMediaFoundationH264)
		{
			GUI.color = Color.yellow;
			GUILayout.TextArea("Warning: This is an experimental encoder.  Make sure to set the file extension to .mp4");
			GUI.color = Color.white;
		}
